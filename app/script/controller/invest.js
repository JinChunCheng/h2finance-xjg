        ...
        $("#iptpwd-ctt .pwd-verify").on("click", function() {
            ...
            if (!pswdVerify) {
                ...
                service.ajax("POST", JSON.stringify({
                        "amount": investNum.val(),// (parseFloat(investNum.val()) + parseFloat(serviceFee.text())).toFixed(),
                        "bankAccount": bankNum,
                        "paymentChannel": "IBOXPAY",
                        "paymentType": "WH",
                        "salesId": 1,
                        "rechargeChannel": 3, // 3代表小金贯
                        "fundPassword": _hcqApp.security.encrypt(pswd.val())
                    }), { timeout: 120000 })
                    .done(function(res) {
                        if (res.code == 200 || res.code == 11003) {
                            var amount = investNum.val();
                            res.data = res.data || {};
                            var date = res.data.workDay || '';
                            var url = 'investSucceed.html?code=' + res.code + '&amount=' + amount + '&date=' + date + '&v=' + Math.random();
